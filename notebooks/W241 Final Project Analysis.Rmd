...
```{r}
model3 = lm(Score ~ (group=="dark_mode")+(group=="low_contrast")+(group=="neon")+as.factor(student_status)+nighttime,data=data)
summary(model3)
```
```{r}
robust_CI_3<- coefci(
...
```{r}
for (col in data[,c("RC_score","Diff_score","M_score","Pattern_score"),by=group]){
  pritn()
}
```
